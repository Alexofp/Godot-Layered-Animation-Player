...
@onready var animTree:LayeredAnimPlayer = $"ModelRoot/HumanModel/LayeredAnimPlayer"
...
func process_physics_gun(_delta):
...
func cam_transitioning(height, offset, length, fov, mesh_visible):
	...
	while is_cam_transitioning:
		...
		var _lerp = (current_time - time)/(TRANSITION_SPEED * 1000.0)
		...
		camera_pivot.position.y = lerp(orig_height, height, ease_in_out_sine(_lerp))
		spring_arm.position.x = lerp(orig_offset, offset, ease_in_out_sine(_lerp))
		# Adjusting spring_length is jittery. Likely only updates on physics process.
		spring_arm.spring_length = lerp(orig_length, length, _lerp)
		# So if you're noticing jitter when switching cams, ^ this lerp is responsible.
		camera.fov = lerp(orig_fov, fov, ease_in_out_sine(_lerp))
...
func switch_anim(anim:String, _speed:float = 1.0):
	#if anim_player.current_animation != anim:
	#	anim_player.play(anim, -1, speed)
	$ModelRoot/HumanModel.playLocomotionAnim(anim, _speed)
...
func ease_in_out_sine(_lerp: float) -> float:
	return -(cos(PI * _lerp) - 1.0) / 2.0

func ease_out_circ(_lerp: float) -> float:
	return sqrt(1.0 - pow(_lerp - 1.0, 2.0))
